...
export const BUILTIN_SHORT_VIDEO_KNOWLEDGE: BuiltInKnowledgeEntry[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      "视频去重处理是独立模式，输入可以是混剪成片、本地 MP4 或其他任务产物，输出保真处理后视频和封面；评分报告不再作为用户操作步骤。",
      "混剪负责重新组合素材，去重负责在成片基础上做二次保真处理；不要把混剪、去重和平台判定承诺混在一个按钮里。"
    ]
  },
  {
    ...
    lines: [
      ...
      "保真去重默认使用标准处理：尽量保持肉眼观感，同时改变光学偏移、局部动态裁切、轻微镜头畸变、帧重采样、像素扰动、GOP、编码和音频结构。",
      "轻度处理适合自有素材二次包装：小幅动态裁切、轻微光学偏移、轻微镜头畸变、轻度色彩/噪声/音频扰动，优先保持观感最接近。",
      "标准处理适合常规成片：动态裁切 + 光学偏移 + 轻微透视扰动 + 镜头畸变 + 帧重采样 + 音频轻扰动，是默认推荐方案。",
      "强处理适合高重复风险素材：在标准处理基础上增加更明显的透视扰动、边缘重采样、像素重塑、帧混合和音频轻回声，但仍避免肉眼明显变形。",
      "高风险时建议替换镜头、补充自有素材、重写脚本、重生成 AI 片段或用视频模型对局部片段做 V2V/图生视频重构。"
    ]
  }
];
