...
export const BUILTIN_SHORT_VIDEO_KNOWLEDGE: BuiltInKnowledgeEntry[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      "视频去重处理是独立模式，输入可以是混剪成片、本地 MP4 或其他任务产物，输出保真处理后视频和内部重复风险/原创度评分报告。",
      "原创概率实现为内部原创度评分，默认通过阈值 80 分；报告必须解释评分原因，不代表平台官方判定，也不承诺任何平台结果。"
    ]
  },
  {
    ...
    lines: [
      ...
      "保真去重默认使用保真强处理：尽量保持肉眼观感，同时改变重采样、帧率、轻微像素扰动、GOP、编码和字幕封面结构。",
