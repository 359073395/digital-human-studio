...
- 统一进度条：所有视频模式在任务栏下方显示生成进度，长时间生成混剪、数字人、去重或导出时不会只停留在按钮 loading；输出时会定时刷新任务状态。
...
- 混剪视频：选择素材文件夹，按素材数量、组合和重复率自动估算批量生成数量；支持固定素材模式和音频模式，音频只作为配音/BGM/时长依据，永远不会作为画面素材使用；同一文件夹内复制出来的多个镜头会作为多个可选素材读取；短视频素材不会在单个片段内部硬循环首帧，画面不足时会继续按素材顺序补下一个片段，减少黑帧闪烁。
