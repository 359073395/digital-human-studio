...
- 混剪视频：选择素材文件夹，按素材数量、组合和重复率自动估算批量生成数量；支持固定素材模式和音频模式，音频只作为配音/BGM/时长依据，永远不会作为画面素材使用；已读取的视频/图片素材和音频素材可以在任务内单独移除；多条音频会按生成批次循环使用；同一文件夹内复制出来的多个镜头会作为多个可选素材读取；短视频素材不会在单个片段内部硬循环首帧，画面不足时会按 1、2、3 文件夹顺序继续补片段，并优先轮换同文件夹内的其他镜头，减少黑帧闪烁和一闪一闪。
